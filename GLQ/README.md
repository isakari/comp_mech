...
が成り立つことはほとんど明らかであろう (証明のスケッチ: $q_k$ を $p_0, p_1, \cdots, p_{k}$ の線型結合で表し, 式 ([@eq:tyokkou]) を用いれば良い). 
...
ここに, $~q_1, r_1$ は $x$ の高々 1 次の多項式である. これを用いれば, 
...
と計算すれば良い. 式 ([@eq:saigo]) の右辺の分子に現れる積分は, 例えば $\left[N/2\right]$ 次の Gauss-Legendre 数値積分で計算すれば良い. 
...
分点 $x_i$ や重み $w_i$ はできるだけ正確に求めておきたいから, 数値計算に通常用いる倍精度実数ではなく, 例えば 4 倍精度実数を用いて計算すると良い. 4 倍精度演算を用いて $x_i$, $w_i$ を求める fortran コードは [https://github.com/isakari/glq_quadp/](https://github.com/isakari/glq_quadp/) を参照されたい. このプログラムを用いて計算した分点と重みの表を以下に示しておく. 

```a.f90
N=1
i   integral point　　　　　　　　　　　　　　　　　　　　　　 　　　weight
...
> (1) $\displaystyle\int_{0}^2 f(x) \mathrm{d}x$ を解析的に計算せよ.
> (2) $\displaystyle\int_{0}^2 f(x) \mathrm{d}x$ を Gauss-Legendre 数値積分を用いて計算し, (1) の結果と比較せよ.
